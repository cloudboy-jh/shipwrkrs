import { SYSTEM_PROMPT, badRequest, getUserLimits, incrementAction, isMockMode, json, makeMockWorkerCode, requireSession, sleep, } from './_lib';
export const onRequestPost = async (context) => {
    ...
    if (tier === 'premium') {
        if (!context.env.ANTHROPIC_API_KEY)
            return badRequest('Premium model unavailable', 503);
        model = 'claude-sonnet';
        ...
        if (!resp.ok)
            return badRequest('Premium generation failed', 502);
    ...
    if (!code)
        return badRequest('Model returned empty code', 502);
    await incrementAction(context.env, session.userId, tier === 'premium' ? 'gen_premium' : 'gen_free');
    const updated = await getUserLimits(context.env, session.userId);
    return json({
        code,
