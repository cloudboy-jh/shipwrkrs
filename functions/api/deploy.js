import { badRequest, cloudflareDeploy, getStoredApiToken, getUserLimits, incrementAction, isMockMode, json, logDeploy, requireSession, sleep, } from './_lib';
...
export const onRequestPost = async (context) => {
    ...
    if (limits.deploysRemaining <= 0)
        return badRequest('Deploy limit hit for today', 429);
    ...
    if (!accessToken)
        return badRequest('No deploy token available. Re-auth or set token once.', 401);
    if (!accountId)
        return badRequest('Account ID unavailable', 400);
