import { badRequest, createSessionToken, ensureUser, getCookie, getDefaultAccountId, setCookie, } from '../_lib';
export const onRequestGet = async ({ request, env }) => {
    ...
    if (!tokenRes.ok)
        return badRequest('Failed to exchange OAuth code', 401);
    ...
    if (!userRes.ok || !userData.result)
        return badRequest('Failed to fetch Cloudflare user', 401);
